...
data_dir = '/Applications/PycharmProject/My_thesis/colab'
...
image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms[x]) for x in ['data']}
...
def drawCM(matrix, savname):
    ...
    font = ImageFont.truetype(os.path.join(os.getcwd(), "/Applications/PycharmProject/My_thesis/colab/11/Arial.ttf"), 15)
    for i in range(lines):
        for j in range(cols):
            dig = str(round(matrix[i, j]*100,2))
            if i == j:
                filled = (255, 181, 197)
            else:
                filled = (46, 139, 87)
...
if __name__ == "__main__":
    ...
    linesum = conf_matrix.sum(1)
    linesum = np.dot(linesum.reshape(-1, 1), np.ones((1, conf_matrix.shape[1])))
    conf_matrix /= linesum
    drawCM(conf_matrix, 'cm5.jpg')
